...
def render_cosmo_form(defaults: dict | None = None, prefix: str = "") -> dict:
    ...
    with st.container(border=True):
        ...
        with c1:
            h = st.number_input(
                "h", value=defaults.get("h", 0.6774), format="%.4f", key=f"{prefix}h"
            )
        with c2:
            omega_b = st.number_input(
                "omega_b",
                value=defaults.get("omega_b", 0.0486),
                format="%.4f",
        ...
        with c3:
            omega_k = st.number_input(
                "omega_k",
                value=defaults.get("omega_k", 0.0),
                format="%.4f",
        ...
        with c4:
            omega_nu = st.number_input(
                "omega_nu",
                value=defaults.get("omega_nu", 0.0),
                format="%.4f",
        ...
        with c5:
            w0 = st.number_input(
                "w0", value=defaults.get("w0", -1.0), format="%.4f", key=f"{prefix}w0"
            )
        with c6:
            wa = st.number_input(
                "wa", value=defaults.get("wa", 0.0), format="%.4f", key=f"{prefix}wa"
            )

        n_s = st.number_input(
            "n_s", value=defaults.get("n_s", 0.9667), format="%.4f", key=f"{prefix}n_s"
        )
