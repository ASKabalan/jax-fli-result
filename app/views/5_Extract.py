...
with st.container(border=True):
    ...
    source = st.radio(
        "Data source", ["Local directory", "HuggingFace repo"], key="ext_source"
    )
    ...
    if source == "Local directory":
        input_dir = st.text_input(
            "input_dir", value="test_fli_samples", key="ext_input_dir"
        )
    else:
        repo_id = st.text_input(
            "repo_id (e.g. 'ASKabalan/jax-fli-experiments')", key="ext_repo_id"
        )
    ...
    truth_parquet = st.text_input(
        "truth_parquet",
        value="test_fli_samples/chain_0/samples/samples_0.parquet",
        key="ext_truth",
    )
    output_file = st.text_input(
        "output_file", value="results/extracts/extract.parquet", key="ext_output"
    )
