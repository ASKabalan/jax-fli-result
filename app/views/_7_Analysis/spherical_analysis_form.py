    ...
    with spec_params_col:
        with st.container(border=True):
            ...
            _ref_label = st.selectbox(
                "Reference entry", _labels,
                index=0, key="analysis_spherical_cl_ref_entry",
            )
            _ref_idx = _labels.index(_ref_label)
            active_entries = (
                [active_entries[_ref_idx]]
            ...
            with lc1:
                lmin = st.number_input(
                    "LMIN", min_value=0,
                    value=_lmin_stored if precomputed else 10,
                    key="analysis_lmin",
                )
            with lc2:
                lmax = st.number_input(
                    "LMAX", min_value=10,
                    value=_lmax_stored if precomputed else int(3 * _default_nside),
                    key="analysis_lmax",
                )

            nonlinear_fn_name = st.selectbox(
                "Nonlinear fn", ["halofit", "linear"], key="analysis_nl_fn",
            )

            # --- Comparison toggles ---
            compare_ref = st.checkbox(
                "Compare against reference", value=False,
                key="analysis_compare_ref",
                disabled=(len(active_entries) < 2),
            )
            compare_theory = st.checkbox(
                "Compare against theory", value=False,
                key="analysis_compare_theory",
            )
            ratio_only = st.checkbox(
                "Ratio only (hide main panel)", value=False,
            ...
            if compare_theory:
                with st.expander("Probe settings", expanded=True):
                    ...
                    probe_type = st.radio(
                        "Probe", _probe_options, key=_pt_key,
                        help=(
                    ...
                    if probe_type == "density":
                        nz_zmax = st.number_input(
                            "nz_zmax", min_value=0.1, value=2.0,
                    ...
                    apply_pixwin = st.checkbox(
                        "Multiply theory by pixel window", value=False,
                        key="analysis_pixwin",
                        help=(
            ...
            with bc1:
                band_2 = st.number_input(
                    "±2% band", min_value=0.0, max_value=100.0,
                    value=2.0, step=1.0, key="analysis_band_2",
                )
            with bc2:
                band_5 = st.number_input(
                    "±5% band", min_value=0.0, max_value=100.0,
            ...
            with cb1:
                compute_btn = st.button(
                    ...
                    key="analysis_compute_btn", type="primary",
                )
            with cb2:
                redraw_btn = st.button(
                    "Redraw", key="analysis_redraw_btn",
    ...
    with spec_plot_col:
        if compute_btn:
            ...
            if precomputed:
                ...
                spectra_results = [
                    (e["label"], e["catalog"].field[0][selected_shells, _lmin_idx:_lmax_idx])
            ...
            if spectra_results:
                ...
                if len(set(spec_lengths)) > 1:
                    st.error("Spectra have different ell lengths after slicing — cannot compare.")
                    st.stop()
                comoving_centers = np.array([s[1].comoving_centers for s in spectra_results])
                ...
                if not np.all(np.isclose(comoving_centers, comoving_centers[0])):
                    st.warning("Spectra have different comoving centers — cannot compare.")
                ...
                if not np.all(np.isclose(density_width, density_width[0])):
                    st.warning("Spectra have different density widths — cannot compare.")
        ...
        if spectra_png:
            ...
            st.info(
                ...
                if precomputed else
                "Click **Compute** to generate angular power spectra."
            )
