...
def pk_tab(
    ...
    with spec_params_pk:
        with st.container(border=True):
            ...
            _ref_label = st.selectbox(
                ...
                index=0, key="analysis_pk_ref_entry",
            )
            ...
            with pk_cb1:
                pk_compute_btn = st.button(
                    ...
                    key="analysis_pk_compute_btn", type="primary",
                )
            with pk_cb2:
    ...
    with spec_plot_pk:
        ...
        if pk_compute_btn:
            if precomputed:
                pk_results   = [(e["label"], e["catalog"].field[0][selected_snaps]) for e in active_entries]
                ...
                if all_types_pk != _DENSITY_3D:
                    ...
                    with st.spinner("Computing 3D power spectra..."):
                        pk_results, ref_fld_pk, ref_cosmo_pk = compute_pk(active_entries, selected_snaps)
            ...
            with st.spinner("Rendering..."):
                with _plt_lock:
                    fig_pk = builder(
                        ...
                        _ref_fld_pk, layout_params, pk_bands,
                    )
        ...
        if pk_png:
            ...
            st.info(
                ...
                if precomputed else
                "Click **Compute** to generate the 3D matter power spectrum."
            )
